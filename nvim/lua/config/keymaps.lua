...
-- === Insert Mode ===
-- Exit Insert Mode
maps.i['jk'] = { '<Esc>', desc = "go to Normal mode" }
maps.i['jj'] = { '<Esc>', desc = "go to Normal mode" }
...
if is_available "telescope.nvim" then
  ...
  maps.n["<leader>fa"] = {
    function()
      ...
      local search_dirs = {}
      for _, dir in ipairs({ vim.fn.stdpath "config" }) do                      -- search all supported config locations
        if dir == vim.fn.stdpath "config" then dir = dir .. "/lua" end          -- don't search the astronvim core files
...
if is_available "toggleterm.nvim" then
  maps.n["<leader>t"] = " Terminal"
  if vim.fn.executable "lazygit" == 1 then
    maps.n["<leader>g"] = "󰊢 Git"
    maps.n["<leader>gg"] = {
      function()
        --local worktree = require("config.git").file_worktree()
...
if is_available "nvim-dap" then
  maps.n["<leader>d"] = " Debugger"
  maps.v["<leader>d"] = " Debugger"
